...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, stride=1):
        ...
        # contains the modules passed through it in the correct sequence
        self.shortcut = nn.Sequential()
        if stride != 1 or in_planes != planes:
            self.shortcut = nn.Sequential(nn.Conv2d(in_planes, planes, kernel_size=1, stride=stride,
...
class ResNet(nn.Module):
    def __init__(self, block, num_blocks, num_classes=10):
        ...
        self.bn1 = nn.BatchNorm2d(64)  # reduces the internal covariate shift
        # make the first layer with the 64 planes
        self.layer1 = self._make_layer(block, 64, num_blocks[0], stride=1)
        # make the second layer with the 128 planes
        self.layer2 = self._make_layer(block, 128, num_blocks[1], stride=2)
        # make the third layer with the 256 layers
        self.layer3 = self._make_layer(block, 256, num_blocks[2], stride=2)
        # make the fourth layer with the 512 planes
